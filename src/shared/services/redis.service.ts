...
export class RedisService {
   ...
   */
  async commandStats() {
    const rawInfo = await this.redis.info('commandstats');
    // 按行分割字符串
    const lines = rawInfo.split('\r\n');
    const commandStats = [];
    // 遍历每一行并分割键值对
  ...
  async lLeftPushIfPresent(key: string, ...val: string[]): Promise<number> {
    if (!key) return 0;
    return await this.redis.lpushx(key, ...val);
  }
  ...
  async lLeftInsert(key: string, pivot: string, val: string): Promise<number> {
    ...
    return await this.redis.linsert(key, 'BEFORE', pivot, val);
  }
  ...
  async lRightInsert(key: string, pivot: string, val: string): Promise<number> {
    if (!key || !pivot) return 0;
    return await this.redis.linsert(key, 'AFTER', pivot, val);
  }
  ...
  async lRightPushIfPresent(key: string, ...val: string[]): Promise<number> {
    if (!key) return 0;
    return await this.redis.rpushx(key, ...val);
  }
  ...
  async lPoplPush(
    ...
    return await this.redis.brpoplpush(sourceKey, destinationKey, timeout);
  }
